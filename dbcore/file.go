...
func OpenDataFile(path string, growth int) (file *DataFile, err error) {
	...
	fmt.Printf("%s opened: %d of %d bytes in-use", file.Path, file.Used, file.Size)
	return
}
...
func (file *DataFile) EnsureSize(more int) (err error) {
	...
	fmt.Printf("%s grown: %d -> %d bytes (%d bytes in-use)", file.Path, file.Size-file.Growth, file.Size, file.Used)
...
func (file *DataFile) Clear() (err error) {
	...
	fmt.Printf("%s cleared: %d of %d bytes in-use", file.Path, file.Used, file.Size)
	return
}
